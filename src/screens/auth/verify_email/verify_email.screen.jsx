...
const VerifyEmail = () => {
  ...
  const sendVerification = async (values, actions) => {
    try {
      const user = await Models.auth.resendVerification(values);
  ...
  const confirmEmail = async (values, actions) => {
    try {
      const user = await Models.auth.confirmEmail(values);
      actions.setSubmitting(false);
      setState({
        alert: true,
        alertMessage: user.message,
  ...
  const formik = useFormik({
    ...
    onSubmit: state.initiated ? confirmEmail : sendVerification,
  });

  const resendOtp = async () => {
    try {
      setState({ resending: true });
      const user = await Models.auth.resendVerification({ email: state.email });
      setState({
        alert: true,
        alertMessage: user.message,
        alertType: "success",
              ...
              >
                ...
                <Box sx={{ py: 2 }}>
                  <Button
                    ...
                    variant="contained"
                  >
                    {state.initiated ? "Verify Email" : "Send OTP"}
                  </Button>
                </Box>
                {state.initiated && (
                  <Countdown
